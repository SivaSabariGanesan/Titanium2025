...
export default function EventDetail({
  ...
  // Debug logging for registration status (remove in production)
  console.log('EventDetail - Registration Status:', {
  ...
  apiEvent && typeof apiEvent.id !== "undefined"
      ? {
          ...
          location: apiEvent.event_mode === "online" 
            ? (apiEvent.venue || "Online Event") 
            : (apiEvent.venue || legacyEvent.location),
          time:
            apiEvent.start_time && apiEvent.end_time
              ? `${formatTimeToAMPM(apiEvent.start_time)} - ${formatTimeToAMPM(apiEvent.end_time)}`
              : legacyEvent.time,
        }
      : legacyEvent;
      ...
      <div className="w-full px-6 py-6 lg:px-12 lg:py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 mb-8 lg:mb-12">
          ...
          <div className="flex-1 lg:flex-[2]">
            {apiEvent?.event_video ? (
              // Display video container with increased height
              <div className="w-full h-[400px] sm:h-[500px] lg:h-[600px] bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden relative shadow-2xl">
                <iframe
                  src={apiEvent.event_video}
                  title={event.title}
              ...
              // Display image without container and without play button
              <div className="w-full h-[400px] sm:h-[500px] lg:h-[600px] bg-black/40 rounded-3xl overflow-hidden relative shadow-2xl">
                <Image
                  src={
                    event.image ||
                    "/placeholder.svg?height=600&width=800&query=event poster"
                  }
                  alt={event.title}
            ...
            <div className="bg-black/30 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 lg:sticky lg:top-8 shadow-2xl">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={(() => {
                    const isUserRegistered = registrationStatus?.is_registered ?? false;
                    const isPaidEvent = apiEvent?.payment_type === 'paid';
                    const hasPaid = registrationStatus?.payment_status ?? false;
                    const isRegistrationOpen = apiEvent?.is_registration_open ?? true;

                    // For paid events, allow access if registered but not paid
                    // For free events, disable if registered
                    const shouldDisable = (isUserRegistered && (!isPaidEvent || hasPaid)) || questionsLoading || !apiEvent || !isRegistrationOpen;
                ...
                >
                  {(() => {
                    ...
                    console.log('Button render - payment_status:', registrationStatus?.payment_status);
                    ...
                    console.log('Button render - is_registration_open:', apiEvent?.is_registration_open);
                    ...
                    const hasPaid = registrationStatus?.payment_status ?? false;
                    const isRegistrationOpen = apiEvent?.is_registration_open ?? true;
                    const isFull = apiEvent?.max_participants && apiEvent?.get_current_participants && apiEvent?.get_current_participants >= apiEvent?.max_participants;

                    if (isFull) {
                      console.log('Showing FULL');
                      return "SLOTS FILLED";
                    } else if (!isRegistrationOpen) {
                      console.log('Showing REGISTRATION CLOSED');
                      return "REGISTRATION CLOSED";
                    } else if (isUserRegistered && isPaidEvent && hasPaid) {
                      console.log('Showing ALREADY REGISTERED (paid and completed)');
                      return "ALREADY REGISTERED";
                    } else if (isUserRegistered && !isPaidEvent) {
                      console.log('Showing ALREADY REGISTERED (free event)');
                      return "ALREADY REGISTERED";
                    } else if (isUserRegistered && isPaidEvent && !hasPaid) {
                      console.log('Showing COMPLETE PAYMENT');
