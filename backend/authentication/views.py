...
def get_or_create_social_app(provider):
    """Get or create a SocialApp instance for the given provider"""
    try:
        social_app = SocialApp.objects.get(provider=provider)
        return social_app
    except SocialApp.DoesNotExist:
        raise ValueError(f"Social app for {provider} not found. Please configure it in Django admin.")
...
def google_callback(request):
    ...
    try:
        ...
        if not email.lower().endswith('@rajalakshmi.edu.in'):
            return Response(
                {"error": "Only REC students with @rajalakshmi.edu.in email addresses are allowed to sign up/sign in."},
...
def refresh_token(request):
    ...
    try:
        ...
        if not user:
            return Response(
                {"error": "Invalid refresh token - no user found"},
                status=status.HTTP_401_UNAUTHORIZED
            )
        ...
        try:
            ...
            return Response(
                {"error": "User not found for the provided refresh token"},
                status=status.HTTP_401_UNAUTHORIZED
            )
        new_refresh = RefreshToken.for_user(user_instance)
        ...
        return Response(
            {"error": f"An error occurred while refreshing the token: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
    ...
    def post(self, request):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                logger.info(f"Created admin user: {user.username} (superuser: {is_superuser})")
